...
scv_ms_exp_cs <- function(process_output,
  ...
  if(!large_n){
    ...
    denoms <- process_output %>%
      mutate(site = 'all sites') %>%
      group_by(site, !!sym(col)) %>%
      summarise(!!denom := sum(!!sym(denom))) %>%
      distinct()
...
scv_ms_anom_cs <- function(process_output,
  ...
  if(!large_n){
    ...
    suppressWarnings(
      far_site <- final %>%
        mutate(zscr = (!!sym(comparison_col) - mean_val) / sd_val,
               ...
               zscr = abs(zscr)) %>%
        group_by(concept_id, source_concept_id) %>%
    ...
    nsite_anom <- final %>%
      group_by(concept_id, source_concept_id, anomaly_yn) %>%
    ...
    sitesanoms <- final %>%
      ...
      select(concept_id, source_concept_id, site_anoms)

    tbl <- final %>%
      group_by(concept_id, source_concept_id) %>%
      mutate(iqr_val = stats::IQR(!!sym(comparison_col))) %>%
      ungroup() %>%
      distinct(concept_id, source_concept_id, mean_val, sd_val, median_val, iqr_val) %>%
...
scv_ms_exp_la <- function(process_output,
  ...
  if(!large_n){
    ...
    site_spec <- process_output %>%
      filter(site %in% large_n_sites) %>%
      inner_join(nmap_top)

    allsite_summ <- process_output %>%
      inner_join(nmap_top) %>%
      select(time_start, col, map_col, all_of(facet), ct, prop,
             concept_name, source_concept_name) %>%
      group_by(time_start, !!sym(col), !!sym(map_col), concept_name,
               source_concept_name) %>%
      summarise(ct = sum(ct),
                med = median(!!sym(prop)),
                q1 = stats::quantile(!!sym(prop), 0.25),
                q3 = stats::quantile(!!sym(prop), 0.75)) %>%
      ungroup() %>%
      distinct() %>%
  ...
  if(!large_n){
    ...
    p <- allsite_summ %>%
      mutate(concept_id = as.character(concept_id),
             source_concept_id = as.character(source_concept_id)) %>%
      ggplot(aes(y = value, x = time_start, color = site,
                 label = !!sym(map_name),
                 label2 = ct
      )) +
...
scv_ms_anom_la <- function(process_output,
  ...
  iqr_dat <- filt_op %>%
    select(time_start,!!sym(concept_col),!!sym(mapped_col),prop_col) %>% distinct() %>%
    group_by(time_start, !!sym(concept_col), !!sym(mapped_col)) %>%
    summarise(q1 = quantile(prop_col, 0.25),
              q3 = quantile(prop_col, 0.75))
